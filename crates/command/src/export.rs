...
impl  BucketInfo {
   fn new (name: String, source: String, updated: String, manifests: u32) -> BucketInfo {
      BucketInfo { Name: name,  Source: source,  Updated: updated, Manifests: manifests }
   }
}
#[derive(Debug , Clone , Serialize, Deserialize )]
struct   InstalledApp {
...
impl  InstalledApp { 
   fn new (name: String, source: String, updated: String, version: String) -> InstalledApp {
      InstalledApp { Name: name,  Source: source,  Updated: updated, Version: version }
   } 
...
fn get_all_installed_apps() -> Vec<InstalledApp> {
   ...
   for entry in read_dir(apps_path).unwrap() {  
     ...
     if ! manifest_file.is_file() { continue; }  
     let content = std::fs::read_to_string(manifest_file).unwrap_or_else( 
       |_ | {  
     ...
     installed_apps.push(InstalledApp::new(name.to_string(), source.to_string(),  updated, version.to_string()));
   }
...
fn get_all_buckets_info() -> Vec<BucketInfo > {
  ...
  for bucket_dir in bucket_path {
    ...
    let bucket_info = BucketInfo::new(bucket_name.to_string(), bucket_source, bucket_updated, manfiests_count);
...
pub fn  export_config_to_current_dir_with_config (file_name: String) {
  ...
  let current_dir = std::env::current_dir().unwrap();
  let  path = current_dir.join(path);
  log::info!("导出所有配置文件到 {}", path.display());
