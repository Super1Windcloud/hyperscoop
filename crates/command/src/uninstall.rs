...
fn rm_start_menu_shortcut(manifest: &Value) -> Result<(), anyhow::Error> {
  let shortcut = manifest.get("shortcuts");
  if shortcut.is_none() {
    return Ok(());
  }
  let shortcuts = shortcut.unwrap().as_array().unwrap();
  if shortcuts.is_empty() {
    return Ok(());
  }
  for shortcut_item in shortcuts {
    let default_array: Vec<Value> = Vec::new();
    let shortcut = shortcut_item.as_array().unwrap_or(&default_array);
    if shortcut.is_empty() {
      continue;
    }
    let arg_len = shortcut.len();
    if arg_len < 2 {
      eprintln!("{} ", "Failed to find shortcut, maybe manifest json file format error".dark_yellow().bold());
      continue;
    }
    let shortcut_name = shortcut[1].as_str().unwrap_or("").to_string()+".lnk";
    ...
    if path.exists() && path.is_dir() {
      ...
      if path.exists() &&  path.is_file()  {
        println!("Removing start menu shortcut '{}'", path.display().to_string().dark_blue().bold());
...
fn rm_shim_file(shim_path: String, manifests: &Value, app_name: &str) -> Result<(), anyhow::Error> {
  ...
  let manifest = manifests.get("bin");
  if manifest.is_none() {
    eprintln!("{} '{}'", "Failed to find shim  on".dark_yellow().bold(), app_name.dark_yellow().bold());
    return Ok(());
  }
  match manifest.unwrap() {
    ...
    Value::Array(a) => {
      for item in a {
        ...
        if suffix == "bat" {
          let bat_file = shim_path.join((prefix.to_string() + ".bat").as_str());
          if bat_file.exists() {
            println!("Removing shim file {}", bat_file.display().to_string().dark_blue().bold());
            std::fs::remove_file(&bat_file)?;
          }
          let cmd_str = prefix.to_string() + ".cmd";
          let shell_file = shim_path.join(prefix);
          let cmd_file = shim_path.join(cmd_str);
        ...
        if suffix == "ps1" {
          let ps1_file = shim_path.join((prefix.to_string() + ".ps1").as_str());
          if ps1_file.exists() {
            println!("Removing shim file {}", ps1_file.display().to_string().dark_blue().bold());
            std::fs::remove_file(&ps1_file)?;
          }
          let cmd_str = prefix.to_string() + ".cmd";
          let shell_file = shim_path.join(prefix);
          let cmd_file = shim_path.join(cmd_str);
