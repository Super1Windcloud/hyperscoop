...
pub fn create_path_shim_file_and_shim_bin<P1 : AsRef<Path>  , P2 :AsRef<Path>>(
    target_path: P1,
    output_dir: P2 , 
) ->  anyhow::Result<()> {
   ...
   if target_name.is_err() {
    ...
    println!("{} {}", "Created shim file  for ".to_string().dark_blue().bold() ,
  ...
  #[cfg(windows)]
    {
        ...
        log::trace!("output driver shim {}", &output_shim_exe.display());
...
mod test_shim {
  ...
  #[test]
    ...
    fn test_create_shims() {
      ...
      create_path_shim_file_and_shim_bin(target_path, output_dir ).unwrap(); 
    }
}
