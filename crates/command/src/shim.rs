...
pub fn  list_all_shims () {
   log::info!("list_all_shims");
  ...
  let mut shims = vec![];
   for entry in shim_path.read_dir().unwrap() {
     ...
     let  shim_path =  shim_path.to_str().unwrap().to_owned() + "\\" +&shim_name +".exe";
      let  shim_source = std::fs::read_to_string(path.clone()).unwrap().replace("path =","");
...
pub fn list_shim_info (name : String ) {
 ...
 pub fn execute_add_shim (name : String, source : String ) {
  let   shim_name = name.clone();
